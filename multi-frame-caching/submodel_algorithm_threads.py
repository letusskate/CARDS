"""
multiple process need to interpret from the first line of the file that causes multiple problems.
global variables are not shared between processes.
so use multiple threads instead.
"""
...
output_dir_path = os.path.join("output_data", f"{local_time}-threads")
input_dir_path = "data_dir"
output_file_name = "multiple_threads_calculate_log.txt"
input_file_name = os.path.join("framedataExample", "frame1Example.txt")
...
def multiple_threads_fn(round_cnt, n, x_ln_now, process_output_file):
    global l_num, x_ln_this, c_opt, A_ub, A_eq, bounds
    server_best_l = -1
    server_best_value = 1e10
    for i in range(l_num):
        time_now = time.time()
        if x_ln_now[i * n_num + n] == 1:
            continue
        x_ln_this = x_ln_now.copy()
        x_ln_this[i * n_num + n] = 1
        this_value = LP_solve(
            x_ln=x_ln_this, c_opt=c_opt, A_ub=A_ub, A_eq=A_eq, bounds=bounds
        )
        txt_file = open(file=process_output_file, mode="a+", encoding="utf-8")
        txt_file.write(
            f"round:{round_cnt}, server:{n + 1}, service:{i + 1}, this-value:{this_value}, time-cost:{time.time() - time_now}, "
        )
        if this_value == "NoSolution":
            txt_file.write(
                f"server-best-service:{server_best_l + 1}, server-best-value:{server_best_value}\n"
            )
            txt_file.close()
            continue
        elif this_value < server_best_value:
            server_best_value = this_value
            server_best_l = i
        txt_file.write(
            f"server-best-service:{server_best_l + 1}, server-best-value:{server_best_value}\n"
        )
        txt_file.close()
    global thread_lst
    thread_lst.append((server_best_value, server_best_l, n))
...
def greedy_algorithm():
    ...
    for _ in tqdm.tqdm(generator()):  # make tqdm able to show the speed
        ...
        min_this_value = 1e9
        min_i = -1
        threads = [threading.Thread()] * n_num
        global thread_lst
        thread_lst.clear()
        for i in range(n_num):
            thread_output_file = output_file.replace(".txt", f"_thread-{i + 1}.txt")
            threads[i] = threading.Thread(
                target=multiple_threads_fn,
                args=(round_cnt, i, x_ln_now, thread_output_file),
            )
            threads[i].start()
            threads[i].join()  # join here actually mean noting
        for i in range(n_num):
            # threads[i].join() # join here sometimes causes bugs
            tup = thread_lst[i]
            this_value = tup[0]
            if this_value < min_this_value:
                min_this_value = this_value
                min_i = tup[1] * n_num + tup[2]
            with open(file=output_file, mode="a+", encoding="utf-8") as txt_file:
                txt_file.write(
                    f"round:{round_cnt}, server:{tup[2] + 1}, server-best-value:{this_value}, round-best-value:{min_this_value}\n"
                )

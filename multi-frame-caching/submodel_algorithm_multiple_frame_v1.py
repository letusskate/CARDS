"""
each LP calculate we calculate all the frames.
but we need to sort the greedy methord.
greedy algorithm must start from the first frame.
Because the benefit of the first frame and the frames after are the same.
but if we firstly calculate the frames after the first, we can not get the award of the placement of the first frame.
In general, we can calculate the problem frame by frame.
"""
...
for f in range(f_num):
    for i in range(l_num):
        for j in range(n_num):
            this_constraint = [0] * f_num * l_num * n_num * m_num * o_num
            for m in range(m_num):
                for o in range(o_num):
                    this_constraint[
                        f * l_num * n_num * m_num * o_num
                        + i * n_num * m_num * o_num
                        + j * m_num * o_num
                        + m * o_num
                        + o
...
def LP_solve(x_ln, c_opt, A_ub, A_eq, bounds):
    ...
    for f in range(f_num):
        for m in range(m_num):
            this_sum = 0
            for i in range(l_num):
                this_sum += x_ln[f * l_num * m_num + i * m_num + m] * r[i]
        ...
        for i in range(l_num):
            for j in range(n_num):
                for m in range(m_num):
                    ...
                    for o in range(o_num):
                        this_sum += (
                            a[i * n_num * m_num + j * m_num + m]
                            * x_ln[f * l_num * m_num + i * m_num + m]
                        )
        ...
        for i in range(l_num):
            for j in range(n_num):
                ...
                for jj in range(n_num):
                    if x_ln[f * l_num * n_num + i * n_num + jj] == 0 or f == 0:
                        min_this_sum = min(
                            min_this_sum,
                            x_ln[f * l_num * n_num + i * n_num + j] * c[i * n_num + j],
                        )
                    else:
                        min_this_sum = min(
                            min_this_sum,
                            x_ln[f * l_num * n_num + i * n_num + j]
                            * cc[i * n_num * n_num + jj * n_num + j],
                        )
...
def greedy_algorithm():
    x_ln_now = []
    for i in range(f_num * l_num * n_num):
        x_ln_now.append(0)
    ...
    for f in range(f_num):
        ...
        for _ in tqdm.tqdm(generator()):  # make tqdm able to show the speed
            ...
            for i in range(l_num * n_num):
                time_now = time.time()
                if x_ln_now[f * l_num * n_num + i] == 1:
                    continue
                x_ln_this = x_ln_now.copy()
                x_ln_this[f * l_num * n_num + i] = 1
                this_value = LP_solve(
    ...
    return [best_min_value_sum, x_ln_now]

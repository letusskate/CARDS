...
class EdgeFtLayer(nn.Module):
    ...
    def __init__(self, v_in_dim, v_out_dim, e_in_dim, e_out_dim):
        ...
        self.W_a = nn.Parameter(
            nn.init.xavier_normal_(
                torch.zeros((2 * v_in_dim + e_in_dim, v_out_dim)), gain=SQRT_TWO
            )
        )
        ...
        self.W_e = nn.Parameter(
            nn.init.xavier_normal_(torch.zeros((v_in_dim, e_out_dim)), gain=SQRT_TWO)
        )
        self.W_ee = nn.Parameter(
            nn.init.xavier_normal_(torch.zeros((e_in_dim, e_out_dim)), gain=SQRT_TWO)
        )
    ...
    def edge_function(self, edges):
        ...
        e = edges.data["e_feat"]
        new_e_feat = (
            torch.matmul(node_src, self.W_e)
            + torch.matmul(node_dest, self.W_e)
            + torch.matmul(e, self.W_ee)
        )
    ...
    def forward(self, g):
        ...
        """
        g.update_all(
            message_func=self.message_function, reduce_func=self.new_node_features
        )
...
class GATNetwork(nn.Module):
    ...
    def __init__(self, layer_features, n_hidden_layer, latent_dim, output_dim):
        ...
        for l in range(0, self.n_layers):
            layer = EdgeFtLayer(
                self.n_dim[l], self.n_dim[l + 1], self.e_dim[l], self.e_dim[l + 1]
            )
